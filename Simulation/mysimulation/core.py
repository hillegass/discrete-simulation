...
from mysimulation.events import (AllOf, AnyOf, Event, Process, Timeout, PRIORITIZED,
                                 NORMAL)
...
class BaseEnvironment(object):
    ...
    For example, mysimulation defines a :class:`Environment` for simulations with
    a virtual time and and a :class:`~mysimulation.rt.RealtimeEnvironment` that
    ...
    def schedule(self, event, priority=NORMAL, delay=0):
        ...
        There are two default priority values, :data:`~mysimulation.events.PRIORITIZED` and
        :data:`~mysimulation.events.NORMAL`.
    ...
    def run(self, until=None):
        ...
        - If it is an :class:`~mysimulation.events.Event`, the method will continue
        ...
        if until is not None:
            if not isinstance(until, Event):
                ...
                until._ok = True
                until._value = None
                self.schedule(until, PRIORITIZED, at - self.now)
...
class Environment(BaseEnvironment):
    ...
    def peek(self):
        ...
        :data:`~mysimulation.core.Infinity` if there is no further event."""
        try:
