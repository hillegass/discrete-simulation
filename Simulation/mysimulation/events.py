"""
This module contains the basic event types used in mysimulation.
...
.. autosummary::
    ...
    ~mysimulation.events.Event
    ~mysimulation.events.Timeout
    ~mysimulation.events.Process
    ~mysimulation.events.AnyOf
    ~mysimulation.events.AllOf
...
class Event(object):
    ...
    def __init__(self, env):
        self.env = env
        """The :class:`~mysimulation.core.Environment` the event lives in."""
        self.callbacks = []
    ...
    def defused(self):
        ...
        :class:`~mysimulation.core.Environment` processes the event (i.e. in
        :meth:`~mysimulation.core.Environment.step()`).
        ...
        processed by the :class:`~mysimulation.core.Environment`.
    ...
    def __and__(self, other):
        """Return a :class:`~mysimulation.events.Condition` that will be triggered if
...
class Timeout(Event):
    """A :class:`~mysimulation.events.Event` that gets triggered after a *delay* has
    passed.
...
class Interruption(Event):
    ...
    *cause* to be thrown into *process*.
    ...
    def __init__(self, process, cause):
        ...
        self.env.schedule(self, PRIORITIZED)
...
class Process(Event):
    ...
    .. note::
       ...
       generators. You can use :meth:~mysimulation.core.Environment.exit() as
       a workaround.
...
class ConditionValue(object):
    """Result of a :class:`~mysimulation.events.Condition`. It supports convenient
...
class AllOf(Condition):
    """A :class:`~mysimulation.events.Condition` event that is triggered if all of
...
class AnyOf(Condition):
    """A :class:`~mysimulation.events.Condition` event that is triggered if any of
...
class Interrupt(Exception):
    ...
    :func:`~mysimulation.events.Process.interrupt()`).
