...
def start_delayed(env, generator, delay):
    ...
    :meth:`~mysimulation.core.Environment.process()` starts a process at the current
    simulation time. This helper allows you to start a process after a delay of
    *delay* simulation time units::

        >>> from mysimulation import Environment
        >>> from mysimulation.util import start_delayed
...
def subscribe_at(event):
    ...
    a :class:`~mysimulation.events.Process` to get notified when it terminates.
